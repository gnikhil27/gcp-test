<?php
    ...
    if((!isset($_REQUEST["code"])) || $_REQUEST["code"]==""){header("Location: ../coupon.php");die("Go to Cupon page and select a coupon");}
    $products = [
        "1" => ["name"=> "Coupon 1","amount"=>100],
        "2" => ["name"=> "Coupon 2","amount"=>200],
        "3" => ["name"=> "Coupon 3","amount"=>200],
        "4" => ["name"=> "Coupon 4","amount"=>300],
        "5" => ["name"=> "CK Enterprise","amount"=>400],
        "6" => ["name"=> "CK University","amount"=>500],
        "420" => ["name"=> "Cheeting","amount"=>10000]
    ];
    function orderExist($var){
		...
		if($conn->connect_errno == 0){$result = $conn->query("SELECT * FROM $tablename WHERE tid='$var'");/*print_r($result );/**/return $result->num_rows==0?false:true;}else return false;
    }
    $code=$_REQUEST["code"]??"420";
    $buying=$products[$code]??$products["420"];
    $config = parse_ini_file("../site.ini");
    $orderid =1;
    while(orderExist($orderid)){
        $orderid = 
            random_int(10000, 9999999);
    }
?>
<!doctype html>
<html>
        ...
        <div class="row">
        	...
        	<div class="col-lg-6 col-sm-5 wow fadeInUp delay-05s">
            	<div class="form">
                    ...
                    <hr>
                        <div class="form-contro nput-text">
                            <b> Cupon Name: <?= $buying["name"]?></b>
                        </div><hr>
                        <div class="form-contro nput-text">
                            <b> Price: <?= $buying["amount"]?></b>
                        </div>
                        <hr>
                        <div class="form-group">
                            <form action="pgRedirect.php" method="post">
                            <div style="display: none;">
                            <input required type="text" name="ORDER_ID" id="ORDER_ID" value="<?= $orderid?>"/>
                            <input required type="text" name="Plan" id="ORDER_ID" value="<?= $buying["name"]?>"/>
                            <input required type="text" name="CUST_ID"  id="CUST_ID" value="<?= $_SESSION["email"]?>"/>
                            <input required type="text" name="INDUSTRY_TYPE_ID"  id="INDUSTRY_TYPE_ID" value="Retail"/>
                            <input required type="text" name="CHANNEL_ID"  id="CHANNEL_ID" value="WEB"/>
                            <input required type="number" name="TXN_AMOUNT"  id="TXN_AMOUNT" value="<?= $buying["amount"]?>"/>
                            </div>
                            <button type="submit" class="form-control input-lg btn btn-lg btn-success">Pay &#8377;<?= $buying["amount"]?> &#8594;</button>
                            </form>     
                        </div>
                    </form>
                </div>	
